...
use super::constant::{API_ROOT_URL, API_SANDBOX_URL, CB_VERSION, OAUTH_ACCESS_TOKEN_URL, OAUTH_AUTHORIZE_URL, USER_AGENT_NAME};
...
impl SecureHttpClientAgent {
    pub(super) fn new(auth: CoinbaseAuth, sandbox: bool, timeout: Duration, oauth2_callback: Option<Arc<dyn CoinbaseAppOAuth2Callback>>) -> Result<Self, Error> {
        let authenticator: Authenticator = match auth {
            ...
            CoinbaseAuth::OAuth { client_id, client_secret, redirect_url, access_token,refresh_token, expires_at } => {
